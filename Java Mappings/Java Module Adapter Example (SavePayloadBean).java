...
public class SavePayloadBean implements SessionBean, Module {
	...
	public ModuleData process(ModuleContext moduleContext, ModuleData inputModuleData) throws ModuleException {
		...
		// These are two parameters that can be passed to the module in adapter
		// parameters.
		boolean skipExecution = getBooleanParam(moduleContext, "skipExecution"); // skipexecution of this module if set
																					// to 'true'
		boolean failOnError = getBooleanParam(moduleContext, "failOnError"); // Will fail the module and stop process if
																				// set to 'true'
		...
		try {
			...
			String error = "[E2] ModuleData does not contain an object that implements the XI message interface; object class is: "
					+ o.getClass().getName();
