...
public class RenombrarBean implements SessionBean, Module {
	...
	public ModuleData process(ModuleContext moduleContext, ModuleData inputModuleData) throws ModuleException {
		...
		if (ruta == null) {
			location.debugT(SIGNATURE,
					"Parámetro 'directorio' no establecido. Por favor revise la configuración del módulo.");
		...
		if (prefijo == null) {
			location.debugT(SIGNATURE,
					"Parámetro 'prefijo' no establecido. Por favor revise la configuración del módulo.");
			audit.addAuditLogEntry(key, AuditLogStatus.ERROR,
					"Parámetro 'prefijo' no establecido. Por favor revise la configuración del módulo.");
			ModuleException me = new ModuleException();
			throw me;
		...
		if (extension == null) {
			location.debugT(SIGNATURE,
					"Parámetro 'extension' no establecido. Por favor revise la configuración del módulo.");
			audit.addAuditLogEntry(key, AuditLogStatus.ERROR,
					"Parámetro 'extension' no establecido. Por favor revise la configuración del módulo.");
			ModuleException me = new ModuleException();
			throw me;
		...
		if (ficheros == null) {
			...
			for (int x = 0; x < ficheros.length; x++) {
				...
				audit.addAuditLogEntry(key, AuditLogStatus.SUCCESS,
						"Nombre fichero de salida = " + nombreFicheroSalida);
